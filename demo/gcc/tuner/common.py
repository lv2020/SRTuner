    ...
    def tune(self, budget, batch_size=1):
        ...
        while 1:
            if self.args.time_limitation:
                with open(f'{self.args.run_dir}/{self.args.env}_{self.args.time_limitation}_multi_SRTuner_{self.args.random_seed}.pkl', 'wb') as f:
                    pickle.dump(self.op_his, f)
                if time.time() - begin > self.args.time_limitation:
                    break
            else:
                with open(f'{self.args.run_dir}/{self.args.env}_{self.args.steps}_multi_SRTuner_{self.args.random_seed}.pkl', 'wb') as f:
                    pickle.dump(self.op_his, f)
